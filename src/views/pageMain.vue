...
<script>
  // 工具
  import {mapGetters, mapActions} from 'vuex'; // mapGetters从state当中获取数据，mapActions修改数据
  import server from '@/services/server'; // 导入接口服务
  import jointD from '@/utils/joint/sy1';
  ...
  export default{
    ...
    methods: {
      ...
      getTopicsFunc() {
          // then成功
        var duixiang = {};
        server.getTopics(duixiang).then((res) => {
          // 这里对返回数据做应用
         //  this.id = res.data.data[0].id;
          let data = { // let是模块内部变量定义
            name: res.data.data[0].id
          };
          this.changeUserXX(data);
          console.log(res);
        }).catch((err) => {
          console.log(err);
        })
