...
const jointD = {
  ...
  loadBtn(cellView) {
    ...
    // var stretchSelf = '<div id="stretchSelf" class="stretch-icon" onmousedown="stretchSelf(centerPaper, this)"><i class="iconfont">&#xe505;</i></div>';
   ...
   * dataObj 改变颜色的类型 fill, outline, fill
    */
  ...
  initLeftPaper() {
    ...
    let cell1 = this.getShape(1);
    let cell2 = this.getShape(2);
    let cell3 = this.getShape(3);
    let cell4 = this.getShape(4);
    console.log(cell1);
    // var cell5=getShape(5);
    // var cell6=getShape(6);
    // var cell7=getShape(7);
    let cell8 = this.getShape(8);
  ...
  addLeftPaperEvent(paper) {
    ...
    paper.on('cell:click', function (cellView, evt, x, y) {
      // 添加中间画图板内容通过clone()
      var role = this.judgeRole(cellView);
  ...
  addCenterPaperEvent(paper) {
    ...
    paper.on('blank:pointerdown', function (evt, x, y) {
      ...
      $(document).on('mousemove', function (event) {
        var px = event.pageX - $('#display_box').offset().left;
        var py = event.pageY - $('#display_box').offset().top;
        if ($('.moving_box')[0]) {
          $('.moving_box').css({
            'left': sx + 'px',
            'top': sy + 'px',
    ...
    paper.on('blank:pointerup', function (evt, x, y) {
      ...
      for (var i = 0, len = centergraph.getCells().length; i < len; i++) {
        ...
        if ((x >= minX) && (x + width <= maxX) && (y >= minY) && (y + height <= maxY)) {
          ...
          var role = this.judgeRole(centerPaper.findViewByModel(centergraph.getCells()[i]));
          if (JSON.stringify(role) === '{}') {
            ...
            var allChilds = role.parent.getEmbeddedCells({deep: true});
    ...
    paper.on('cell:pointerdown', function (cellView, evt, x, y) {
      ...
      var parent = cellView.model.getAncestors()[cellView.model.getAncestors().length - 1];
      if (parent) {
        cellView.pointerup(evt);
        centerPaper.sourceView = centerPaper.findViewByModel(parent);
        var localPoint = centerPaper.snapToGrid({x: evt.clientX, y: evt.clientY});
        centerPaper.findViewByModel(parent).pointerdown(evt, localPoint.x, localPoint.y);
      }
      // 点击cell右侧属性面板显示对应的数据
      // cell类型
      var type = cellView.model.get('type').split('.')[1].toLowerCase();
  ...
  embed() {
    ...
    var parent = this.getParent(cells[0]);
    var newCells = [];
    ...
    // 删除parent下的所有子cell
    var allChilds = parent.getEmbeddedCells({deep: true});
    if (allChilds) {
  ...
  judgeRole(cellView) {
    var parent = cellView.model.getAncestors()[cellView.model.getAncestors().length - 1];
    var child = cellView.model.getEmbeddedCells({deep: true});
    if (parent) {
  ...
  // 点击delSelf按钮时，删除元素,此前出现了一个错误的思路，导致需要点击两次才能产生效果，如今已经修正
  delSelf() {
    // 除删除cell之外，还要删除与之相关的groups中的数据（根据每个cell的父cell的id），localstorage中的数据，angle的数据
    console.log('delete');
    var parent = this.getParent(view.model);
    var ids = [];
    var allChilds = parent.getEmbeddedCells({deep: true});
    for (var j = 0; j < allChilds.length; j++) {
  ...
  copySelf() {
    ...
    var allCells = this.getAllCells(view);
    var clone = centerPaper.findViewByModel(allCells[0]).model.clone({deep: true});
    centergraph.addCells(clone);
    var apIds = []; // 所有被clone的有关cell的id
    var childsIndex = [];
    var nGroups = []; // 所有被clone的有关的groups中的数据
    ...
    // cell的位置来写clone出来的位置
    for (var l = 0; l < nGroups.length; l++) {
      var parentIndex = apIds.indexOf(nGroups[l].parent);
      groups.push({parent: clone[parentIndex].id, childs: []});
      childsIndex.push([]);
      for (var m = 0; m < nGroups[l].childs.length; m++) {
        childsIndex[l].push(apIds.indexOf(nGroups[l].childs[m].id));
      }
    }

    var index = 0;
    for (var j = groups.length - childsIndex.length; j < groups.length; j++) {
      for (var i = 0; i < childsIndex[index].length; i++) {
  ...
  rotateSelf(centerPaper, e) {
    ...
    $(document).on('mousemove', function (event) {
      var oPos = this.creatWrapper(centerPaper.findViewByModel(allCells[0]), centerPaper);
      var ox = (oPos.maxX - oPos.minX) / 2 + oPos.minX;
      var oy = (oPos.maxY - oPos.minY) / 2 + oPos.minY;
      // console.log(view.getBBox());
      // 旋转中心在中间画布的位置
      px = event.pageX - $('#display_box').offset().left;
      py = event.pageY - $('#display_box').offset().top;
      var angle1 = Math.atan(((ry - oy) / (rx - ox))) * 180 / Math.PI;
      var angle2 = Math.atan(((py - oy) / (px - ox))) * 180 / Math.PI;
      // 一四象限角度需要经过处理
      if (px > ox && py > oy) {
        angle2 = angle2 - 180;
      } else if (px > ox && py < oy) {
  ...
  stretchSelf(centerPaper, e) {
    ...
    var id = $(e).attr('id').substr($(e).attr('id').length - 1);
    $(document).on('mousemove', function (event) {
      // 放大宽高
  ...
  // 获取元素初始化样式,鼠标的事件
  getLeftElementView() {
    let ElementView1 = joint.dia.ElementView.extend({
   ...
   *estroke   尾部分的边框颜色
   */
  getLink(sx, sy, ex, ey, strokecolor, scolor, sstyle, ecolor, estyle, pstrokewidth, sstroke, estroke, plabel) {
  ...
  generateShapes() {
    ...
    // var paper_height = paperOptions.height;
    ...
    let lastCellMaxX = this.getBorderXY1.maxX;
    let lastCellMinX = this.getBorderXY1.minX;
    let lastCellMaxY = this.getBorderXY1.maxY;
    let lastCellMinY = this.getBorderXY1.minY;
    // let getBorderXY2 = this.getBorder(leftPaper.findViewByModel(clone[0]), leftPaper);
    let cloneCellMaxX = this.getBorderXY2.maxX;
    let cloneCellMinX = this.getBorderXY2.minX;
    // var cloneCell_maxY = getBorderXY2.maxY;
    // var cloneCell_minY = getBorderXY2.minY;
  ...
  // connect() {
  //   let socket = new SockJS('http://192.168.1.149:8080/gs-guide-websocket');
